...
export const CAT_COLOR: Record<string, string> = {
  ...
  kids: "#f472b6", // pink
  // Was #f472b6 — the SAME hex as `kids`, not merely close to it, so a pet
  // expense and a child expense were literally indistinguishable on the donut,
  // the budget bars and every ledger chip. Moved to the one part of this
  // palette nothing else occupies: a dark olive. Nearest neighbour is now `car`
  // at ΔE 11.7 (normal vision) against 0.0 before. Under colour-blindness it
  // sits 5.5 from `shopping`, which is inside the band that REQUIRES a second
  // encoding — every row that shows a category colour in this app also shows
  // that category's icon and its written name, so that condition is met.
  pets: "#8b7b00", // dark olive — dog / pets
