...
> - **⭐ NEXT TASKS:** repaint the reused OnePager sheet *interiors* bento (chrome is bento, interiors still graphite: LedgerSheet/PayBillSheet/AccountsSheet/SprintSheet/MarkSentSheet/SettingsSheet/AddTransactionSheet/ImportSheet); wire feed-income recording (income deposits are currently skipped → needed for real anomaly/check tracking); Sam's-Club groceries+household split.
