...
> - **⭐ FLAGGED for a deliberate call (NOT done — structural/risky on a live app):** **(1) Retire the `?oldui` OnePager** (~2335 lines, static-imported so it ships to every user). To retire: EXTRACT the 5 live-reused sheets (`SprintSheet`/`MarkSentSheet`/`AccountsSheet`/`SettingsSheet`/`PayBillSheet`) + their OnePager-private helpers (`shortDebt`/`fmtFull`/`fmtDay`/`Eyebrow`/`Line`/`Stat`/`PaymentSheet`/`ConnectBank`/`CreditCardLinks`/`CreditCardRow`) into their own file(s), repoint `FinanceTabs.tsx:20-26`, then delete `OnePager.tsx` + the `App.tsx` oldUI branch + the OnePager-only graphite CSS (`.hero-bar/.slip-recess/.breathe/.ring-pulse/.pop/.reveal`+`useReveal`, `--color-steel`). Cheaper interim: `React.lazy(OnePager)` to code-split the fallback out of the main bundle. **(2) Goals subsystem** (`SavingsGoal`/`data.goals`/`addGoal`/`contributeGoal`) — loaded+persisted but NEVER read by any view; remove if confirmed unwanted. **(3) Dead store methods** `addDebt`/`payDebt`/`setPaidBill` (zero callers; live paths are card-linking / `payDebtExtra` / `markBillPaid`). **(4)** DDL drops (the `pending_preview` table, `consecutive_failures` column) — I can apply via the Management API. **(5)** Minor dup: `ProfileTab` local `Segmented` vs `ui.tsx` `Segmented`; `ActivityTab` inline money fmt vs `lib/format`; the `buildVMs`↔OnePager assembly dup (dies with `?oldui`). **(6)** Stale docs `docs/STATUS.md` + `docs/debt-attack-plan.html`; archive `_homebase_backup_*.json` off the project dir. **(7)** Carried: repaint reused OnePager sheet *interiors* bento; feed-income recording; Sam's-Club split. If Gino sends family support via **Remitly** (not just the China debt), revisit "every Remitly = the debt".
